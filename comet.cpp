...
void CTrail::onTick() {
  static auto *const PHISTORYSTEP =
      &HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprtrails:history_step")
           ->intValue;
  static auto *const PHISTORYPOINTS =
      &HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprtrails:history_points")
           ->intValue;

  m_iTimer++;

  if (m_iTimer > *PHISTORYSTEP) {
    m_dLastGeoms.push_front({box{(float)m_pWindow->m_vRealPosition.vec().x,
                                 (float)m_pWindow->m_vRealPosition.vec().y,
                                 (float)m_pWindow->m_vRealSize.vec().x,
...
CTrail::CTrail(CWindow *pWindow)
    : IHyprWindowDecoration(pWindow), m_pWindow(pWindow) {
  m_vLastWindowPos = pWindow->m_vRealPosition.vec();
  m_vLastWindowSize = pWindow->m_vRealSize.vec();

  pTickCb = g_pHookSystem->hookDynamic(
      "trailTick", [this](void *self, SCallbackInfo &info, std::any data) {
        this->onTick();
      });
}
...
void CTrail::draw(CMonitor *pMonitor, float a, const Vector2D &offset) {
  ...
  static auto *const PBEZIERSTEP =
      &HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprtrails:bezier_step")
           ->floatValue;
  static auto *const PPOINTSPERSTEP =
      &HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprtrails:points_per_step")
           ->intValue;
  static auto *const PCOLOR =
      &HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprtrails:color")
           ->intValue;
  ...
  box thisbox = box{(float)m_pWindow->m_vRealPosition.vec().x,
                    (float)m_pWindow->m_vRealPosition.vec().y,
                    (float)m_pWindow->m_vRealSize.vec().x,
                    (float)m_pWindow->m_vRealSize.vec().y};
  CBox wlrbox = {thisbox.x - pMonitor->vecPosition.x,
                 thisbox.y - pMonitor->vecPosition.y, thisbox.w, thisbox.h};
  wlrbox.scale(pMonitor->scale).round();

  g_pHyprOpenGL->scissor(
      (CBox *)nullptr); // allow the entire window and stencil to render
  glClearStencil(0);
  ...
  g_pHyprOpenGL->renderRect(&wlrbox, CColor(0, 0, 0, 0),
                            m_pWindow->rounding() * pMonitor->scale);
  ...
  wlr_matrix_project_box(
      matrix, monbox.pWlr(),
      wlr_output_transform_invert(WL_OUTPUT_TRANSFORM_NORMAL), 0,
      g_pHyprOpenGL->m_RenderData.pMonitor->output
          ->transform_matrix); // TODO: write own, don't use WLR here
  ...
  glUseProgram(g_pGlobalState->cometShader.program);

#ifndef GLES2
  glUniformMatrix3fv(g_pGlobalState->cometShader.proj, 1, GL_TRUE, glMatrix);
#else
  wlr_matrix_transpose(glMatrix, glMatrix);
  glUniformMatrix3fv(g_pGlobalState->cometShader.proj, 1, GL_FALSE, glMatrix);
#endif
  ...
  float originalCoeff[2] = {m_pWindow->m_vRealSize.vec().x,
                            m_pWindow->m_vRealSize.vec().y};
  ...
  Vector2D mainVec = {(originalCoeff[0] + 20) / 2 / pMonitor->vecSize.x,
                      (originalCoeff[1] + 20) / 2 / pMonitor->vecSize.y};
  Vector2D windowMiddle = m_pWindow->middle() - pMonitor->vecPosition;

  /*
    points.push_back(Vector2D{cos(0) * mainVec.x - sin(0) * mainVec.y +
                                  windowMiddle.x / pMonitor->vecSize.x,
                              sin(0) * mainVec.x + cos(0) * mainVec.y +
                                  windowMiddle.y / pMonitor->vecSize.y});
    points.push_back(
        Vector2D{cos(-M_PI_2) * mainVec.x - sin(-M_PI_2) * mainVec.y +
                     windowMiddle.x / pMonitor->vecSize.x,
                 sin(-M_PI_2) * mainVec.x + cos(-M_PI_2) * mainVec.y +
                     windowMiddle.y / pMonitor->vecSize.y});
    points.push_back(Vector2D{cos(M_PI_2) * mainVec.x - sin(M_PI_2) * mainVec.y
    + windowMiddle.x / pMonitor->vecSize.x, sin(M_PI_2) * mainVec.x +
    cos(M_PI_2) * mainVec.y + windowMiddle.y / pMonitor->vecSize.y});
    points.push_back(Vector2D{cos(M_PI) * mainVec.x - sin(M_PI) * mainVec.y +
                                  windowMiddle.x / pMonitor->vecSize.x,
                              sin(M_PI) * mainVec.x + cos(M_PI) * mainVec.y +
                                  windowMiddle.y / pMonitor->vecSize.y});
  */
  ...
  for (float t = tCoeff; t <= 1.0; t += tCoeff) {
    ...
    for (size_t i = 1; i < pointsPerBezier + 1; ++i) {
      ...
      const Vector2D &middle =
          vecForT(lastprevbezier, lastbezier, bezierPointStep * (i + 1));
      const Vector2D &lastmiddle =
          vecForT(lastprevbezier, lastbezier, bezierPointStep * i);

      Vector2D vecNormal = {middle.x - lastmiddle.x, middle.y - lastmiddle.y};

      // normalize vec
      float invlen = 1.0 / std::sqrt(vecNormal.x * vecNormal.x +
      ...
      float approxAge = agesForBezier[static_cast<int>(ageFloor)] +
                        (agesForBezier[static_cast<int>(ageCeil)] -
                         agesForBezier[static_cast<int>(ageFloor)]) *
                            (ageCoeff - ageFloor);
      ...
      float coeff = std::min(m_pWindow->m_vRealSize.vec().x / pMonitor->scale,
                             m_pWindow->m_vRealSize.vec().y / pMonitor->scale) *
                    std::exp(1.0 - (approxAge / maxAge));
      Vector2D newVec = {vecNormal.x * coeff / pMonitor->vecSize.x,
                         vecNormal.y * coeff / pMonitor->vecSize.y};
      ...
      points.push_back(
          Vector2D{cos(M_PI_2) * newVec.x - sin(M_PI_2) * newVec.y +
                       middle.x / pMonitor->vecSize.x,
                   sin(M_PI_2) * newVec.x + cos(M_PI_2) * newVec.y +
                       middle.y / pMonitor->vecSize.y});
      points.push_back(
          Vector2D{cos(-M_PI_2) * newVec.x - sin(-M_PI_2) * newVec.y +
                       middle.x / pMonitor->vecSize.x,
                   sin(-M_PI_2) * newVec.x + cos(-M_PI_2) * newVec.y +
  ...
  box thisboxopengl = box{
      (m_pWindow->m_vRealPosition.vec().x - pMonitor->vecPosition.x) /
          (pMonitor->vecSize.x),
      (m_pWindow->m_vRealPosition.vec().y - pMonitor->vecPosition.y) /
          pMonitor->vecSize.y,
      (m_pWindow->m_vRealPosition.vec().x + m_pWindow->m_vRealSize.vec().x) /
          pMonitor->vecSize.x,
      (m_pWindow->m_vRealPosition.vec().y + m_pWindow->m_vRealSize.vec().y) /
          pMonitor->vecSize.y};
  glUniform1i(g_pGlobalState->cometShader.time,
              std::chrono::system_clock::now().time_since_epoch().count());
  glUniform2f(g_pGlobalState->cometShader.output, pMonitor->vecSize.x,
              pMonitor->vecSize.y);
  glUniform4f(g_pGlobalState->cometShader.gradient, thisboxopengl.x,
              thisboxopengl.y, +thisboxopengl.w, thisboxopengl.h);
  glUniform4f(g_pGlobalState->cometShader.color, COLOR.r, COLOR.g, COLOR.b,
              COLOR.a);

  CBox transformedBox = monbox;
  transformedBox.transform(
      wlr_output_transform_invert(
          g_pHyprOpenGL->m_RenderData.pMonitor->transform),
      g_pHyprOpenGL->m_RenderData.pMonitor->vecTransformedSize.x,
      g_pHyprOpenGL->m_RenderData.pMonitor->vecTransformedSize.y);

  glVertexAttribPointer(g_pGlobalState->cometShader.posAttrib, 2, GL_FLOAT,
                        GL_FALSE, 0, (float *)points.data());

  glEnableVertexAttribArray(g_pGlobalState->cometShader.posAttrib);
  ...
  // TRYING TO CREATE SOME BORDER
  //  static auto *const PBORDERSIZE =
  //      &HyprlandAPI::getConfigValue(PHANDLE,
  //      "general:border_size")->intValue;
  //  *PBORDERSIZE = 5;
  //  CBox fullBox = {m_vLastWindowPos.x * pMonitor->scale,
  //                  m_vLastWindowPos.y * pMonitor->scale,
  //                  m_vLastWindowSize.x * pMonitor->scale,
  //                  m_vLastWindowSize.y * pMonitor->scale};
  ...
  //     &fullBox, COLOR, m_pWindow->rounding() * pMonitor->scale + 20, 20, a);

  glDisableVertexAttribArray(g_pGlobalState->cometShader.posAttrib);
...
void CTrail::updateWindow(CWindow *pWindow) {
  ...
  const auto WORKSPACEOFFSET = PWORKSPACE && !pWindow->m_bPinned
                                   ? PWORKSPACE->m_vRenderOffset.vec()
                                   : Vector2D();
...
void CTrail::damageEntire() {
  CBox dm = {(int)(m_vLastWindowPos.x - m_seExtents.topLeft.x),
             (int)(m_vLastWindowPos.y - m_seExtents.topLeft.y),
             (int)(m_vLastWindowSize.x + m_seExtents.topLeft.x +
                   m_seExtents.bottomRight.x),
